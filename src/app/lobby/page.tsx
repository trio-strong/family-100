          ...
          <div className="flex justify-center items-center w-full h-full bg-black bg-opacity-50">
            <div className="flex flex-col w-1/4 h-1/2 p-4 bg-white rounded-[40px] shadow-2xl shadow-black">
              <div className="relative flex justify-center items-center w-full h-1/4 ">
                <button className="absolute top-0 right-0 p-2" onClick={() => setShowCreateRoom(false)}>
                ...
                <div className="flex justify-center items-center gap-2  ">
                  ...
                  <div className="flex justify-center items-center  text-3xl font-extrabold text-[#001b4d] ">Create Room</div>
                </div>
              </div>
      ...
      <div className="flex flex-col max-h-screen w-full px-20 pb-20 overflow-y-auto">
          ...
          <div className="flex w-full ">
            <div className="flex justify-between items-center w-full ">
              <label className="input input-bordered flex max-w-96 h-9 w-full items-center gap-2">
                <input type="text" className="grow italic font-extrabold tracking-widest rounded-xl pl-5 outline outline-2 outline-[#001b4d] h-full" placeholder="Search Room..." />
                <svg
              ...
              <div className="flex justify-center rounded-2xl outline outline-3 outline-[#001b4d]items-center max-w-24 w-full h-10 font-black text-[#001b4d] bg-[#ffbf00]"><button onClick={() => setShowCreateRoom(true)}>+ CREATE</button></div>
            </div>
          </div>
        </div>
        <div className=" flex-col flex  items-center w-full min-h-[73vh] px-3 gap-10 rounded-3xl  bg-white  shadow-2xl shadow-black">
          <div className="flex flex-wrap   w-full overflow-y-scroll my-2 px-10 pb-10 pt-5 gap-4 ">
            {rooms.map((room: any) => (
              <div key={room.id} className="flex max-w-xl w-full h-16 rounded-2xl outline outline-4 outline-[#001b4d] bg-[#ffbf00]">
                ...
                <div className="flex min-w-16 h-full justify-center items-center rounded-r-[10px] px-5 gap-3 bg-[#001b4d]">
                  <Image src={DoorIcon} alt="User Icon" className="flex w-8 h-auto" />
                  <button className="font-extrabold text-2xl text-white" onClick={() => joinRoom(room.id)}>JOIN</button></div>
              </div>
            ))}
          </div>
        </div>
